...
| 节点 | 时机 | 做什么 | 铁律 |
|------|------|--------|------|
| **D1 设计决策** | spec 之前 | 学习性取舍八项：奖励设计 · 信用分配（MC/TD/n-step/λ）· 探索策略 · 对手配置与非平稳性 · 状态表示/输入编码 · 网络结构与输出头 · 评估协议（seed 数/对手/局数）· 训练预算 | 老师列选项 + 权衡，**用户拍板**；不拍板不进 spec |

> D1 清单即真实 RL 决策空间，各实验按需取用。两个注意：
> ① 探索策略不是独立旋钮，形态随算法变（ε-greedy → 熵正则 → MCTS 访问计数），
> 学习主线是探索责任从动作选择转移到算法结构本身；② 对手配置的深层问题是
> "练出来的是会下棋还是会赢这个对手"——课程化与过拟合固定对手都要讨论。
> 概念索引：**致命三要素**（自举+函数近似+异策略，E1-E3 处理）。
